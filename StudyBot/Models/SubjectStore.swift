//
//  SubjectStore.swift
//  StudyBot
//
//  Created by Blake Buddingh on 22/3/22.
//

import Foundation
import SwiftUI

class SubjectStore: ObservableObject {
    @Published var subjects: [Subjects] = []
    
    private static func fileURL() throws -> URL {
        try FileManager.default.url(for: .documentDirectory, in: .userDomainMask, appropriateFor: nil, create: false)
            .appendingPathComponent("subjects.data")
    }
    
    /// Controls saving, and loading of Subjects JSON File
    static func load(completion: @escaping (Result<[Subjects], Error>)->Void) {
        DispatchQueue.global(qos: .background).async {
            do {
                let fileURL = try fileURL()
                guard let file = try? FileHandle(forReadingFrom: fileURL) else {
                    DispatchQueue.main.async {
                        completion(.success([]))
                    }
                    return
                }
                let subjectS = try JSONDecoder().decode([Subjects].self, from: file.availableData)
                DispatchQueue.main.async {
                    completion(.success(subjectS))
                }
            } catch {
                DispatchQueue.main.async {
                    completion(.failure(error))
                }
            }
        }
    }
    
    static func save(subjects: [Subjects], completion: @escaping (Result<Int, Error>)->Void) {
        DispatchQueue.global(qos: .background).async {
            do {
                let data = try JSONEncoder().encode(subjects)
                let outfile = try fileURL()
                try data.write(to: outfile)
                DispatchQueue.main.async {
                    completion(.success(subjects.count))
                }
            } catch {
                DispatchQueue.main.async {
                    completion(.failure(error))
                }
            }
        }
    }
}
